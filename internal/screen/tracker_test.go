...
func init() {
	...
	draw.Draw(img2, img2.Bounds(), &image.Uniform{color.RGBA{255, 255, 255, 255}}, image.ZP, draw.Src)
...
func TestTracker_TrackChanges_DifferentScreenCapture(t *testing.T) {
	...
	capture := func(int) (*image.RGBA, error) {
	...
	tracker := NewTracker(0).WithAlert(playAlert)
	tracker.captureDisplay = capture

	ctx, cancel := context.WithTimeout(context.Background(), time.Millisecond+10)
	defer cancel()

	tracker.TrackChanges(ctx, time.Millisecond)
